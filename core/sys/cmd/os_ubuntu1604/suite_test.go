...
func checkOSVersion(t *testing.T) {
  ...
  if distro != msys_cmd_vars.UBUNTU1604 {
    t.Skip("Skip test")
  }
}
...
func TestCmdSuite_GetMainInterfaceIpAddress(t *testing.T) {
  ...
  cmd2 := "ifconfig " + mainInterface + " | grep 'inet addr' | cut -d ':' -f 2 | awk '{print $1}'"
...
func TestCmdSuite_ConfigureBridgeInterface(t *testing.T) {
  ...
  cmd2 := "ifconfig " + interfaceName + " | grep 'inet addr' | cut -d: -f2 | awk '{print $1}'"
  ...
  cmd3 := "ifconfig " + interfaceName + ` | grep Mask | cut -d: -f4`
...
func TestCmdSuite_UpBridgeInterface(t *testing.T) {
  ...
  cmd2 := "ifconfig " + interfaceName + ` | sed -n 3p | awk '{print $1}'`
  output2, err := exec.Command("sh", "-c", cmd2).Output()
  if err != nil {
  ...
  // check the result
  state := strings.Trim(string(output2), "\n")
...
func TestCmdSuite_AddRouteToIp(t *testing.T) {
  checkOSVersion(t)
  destIP := "172.217.0.46"
  gatewayIP := "123.45.67.89" // hard-coded gateway for testing
  // add route to IP
  ...
  cmd1 := "route -n | grep " + destIP
  output1, err := exec.Command("sh", "-c", cmd1).Output()
  splits := strings.Fields(string(output1))
  destIP2 := splits[0]
  gatewayIP2 := splits[1]
  netmask := splits[2]
